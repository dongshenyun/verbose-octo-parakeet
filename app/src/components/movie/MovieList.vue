...
<script>
import Vue from "vue";//url用Vue.config 需要导入Vue
...
export default {
  ...
  methods: {
    load() {
      axios
    //   .get(Vue.config.url+'http://m.maoyan.com/movie/list.json?type=hot&offset=0&limit=10')
        .get(`./static/data/moviedata${this.movieList.length}.json`)
        .then(res => {
